...
export function QAForm({ qa, onSuccess, className }: QAFormProps) {
  ...
  const [state, formAction] = useFormState((prevState: any, formData: FormData) => submitQA(qa, prevState, formData), initialState);
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            {state.fieldErrors.question?.map((error, i) => (
              <p key={i} id="question-error" className="text-sm text-destructive mt-1">{error}</p>
            ))}
          </div>
          <div className="space-y-2">
            ...
            {state.fieldErrors.answer?.map((error, i) => (
              <p key={i} id="answer-error" className="text-sm text-destructive mt-1">{error}</p>
            ))}
          </div>
          <div className="space-y-2">
            ...
            {state.fieldErrors.tags?.map((error, i) => (
              <p key={i} id="tags-error" className="text-sm text-destructive mt-1">{error}</p>
            ))}
          </div>
          <div className="space-y-2">
            ...
            {state.fieldErrors.importance?.map((error, i) => (
              <p key={i} className="text-sm text-destructive mt-1">{error}</p>
            ))}
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            {state.fieldErrors['metadata.subject']?.map((error, i) => (
              <p key={i} id="subject-error" className="text-sm text-destructive mt-1">{error}</p>
            ))}
          </div>
          <div className="space-y-2">
            ...
            {state.fieldErrors['metadata.context']?.map((error, i) => (
              <p key={i} id="context-error" className="text-sm text-destructive mt-1">{error}</p>
            ))}
          </div>
        </CardContent>
        <CardFooter>
