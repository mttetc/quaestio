...
export async function updateQuestionAnalytics(
    ...
    const existing = await db.query.questionAnalytics.findFirst({
        where: and(eq(questionAnalytics.userId, userId), eq(questionAnalytics.questionHash, questionHash)),
    });
    ...
    if (dateRange) {
        ...
        if (from && to) {
            query = db.query.questionAnalytics.findMany({
                where: and(eq(questionAnalytics.userId, userId), between(questionAnalytics.lastSeen, from, to)),
                orderBy: [desc(questionAnalytics.occurrences)],
                limit,
            });
        }
